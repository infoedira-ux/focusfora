...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function loadUser() {
      ...
      if (lastStudy !== today) {
        ...
        const newStreak = lastStudy === yesterdayStr ? (profile?.study_streak || 0) + 1 : 1;
  ...
  const levelLabels: Record<string, string> = {
    cbc: "CBC Primary", kcse: "KCSE Student",
    university: "University", professional: "Professional Exams",
  };
  ...
  // Ordered by curiosity science — discovery first, duty last
  const sections = [
    {
      icon: "📖",
      title: "Set Books",
      desc: "Chapter summaries, themes, characters & essays",
      href: "/setbooks",
      color: "bg-aubergine",
      textLight: true,
      featured: true,
    },
    {
      icon: "📚",
      title: "Read",
      desc: "70,000+ free books from every era",
      href: "/read",
    ...
    },
    {
      icon: "🧠",
      title: "Research",
      desc: "Millions of academic papers, simplified",
      href: "/research",
      color: "bg-saffron-pale",
      textLight: false,
    },
    {
      icon: "✅",
      title: "Revise",
      desc: "KCSE past papers & revision resources",
      href: "/revise",
    ...
    },
    {
      icon: "🌱",
      title: "CBC Guides",
      desc: "Every grade, every strand covered",
      href: "/cbc",
      color: "bg-aubergine/5",
      textLight: false,
    },
          ...
          { href: "/schools", icon: "🏫", label: "Schools", desc: "Schools directory" },
    {
      icon: "📰",
      title: "Magazine",
      desc: "Vanva Publications — 3 issues",
      href: "/magazine",
      color: "bg-saffron",
      textLight: false,
    },
    ...
    <div className="min-h-screen bg-offwhite">
      <nav className="bg-aubergine px-6 md:px-12 py-4 flex justify-between items-center">
        ...
        <div className="flex items-center gap-4">
          <span className="text-offwhite/50 text-xs hidden md:block">{profile?.email || user?.email}</span>
          <span className="text-offwhite/30 text-xs hidden md:block">·</span>
      ...
      <div className="px-6 md:px-12 py-10 max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
          ...
          <div className="bg-white border border-aubergine/[0.06] rounded-sm p-5">
            ...
            <div className="text-aubergine/40 text-xs mt-1">Upgrade →</div>
          </div>
          <div className="bg-white border border-aubergine/[0.06] rounded-sm p-5">
        ...
        {/* Learning Space */}
        <h2 className="font-serif text-2xl text-aubergine mb-1 font-light">Your Learning Space</h2>
        <p className="text-[#5A4060] text-xs font-light mb-5">Curated for curiosity — explore freely</p>

        {/* Featured Set Books — full width */}
        <Link href="/setbooks"
          className="block bg-aubergine rounded-sm p-8 mb-4 hover:shadow-xl hover:shadow-aubergine/20 hover:-translate-y-1 transition-all duration-300 group">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-saffron text-xs uppercase tracking-widest mb-2">✦ Featured · Most Popular</p>
              <h3 className="font-serif text-3xl text-offwhite mb-2">📖 Set Books</h3>
              <p className="text-offwhite/60 text-sm font-light">
                Chapter summaries · Themes · Character analysis · Essay questions · Exam quotes
              </p>
              <p className="text-offwhite/40 text-xs mt-2">
                Ngũgĩ · Imbuga · Shakespeare · Ogola · Brecht · Ole Kulet and more
              </p>
            </div>
            <span className="text-saffron text-3xl group-hover:translate-x-2 transition-transform">→</span>
          </div>
        </Link>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {sections.slice(1).map((s) => (
            <Link key={s.href} href={s.href}
              ...
              <div className={`font-serif text-lg font-semibold mb-1 ${s.textLight ? "text-offwhite" : "text-aubergine"}`}>
                {s.title}
              </div>
              <div className={`text-xs font-light mb-3 ${s.textLight ? "text-offwhite/60" : "text-[#5A4060]"}`}>
                {s.desc}
              </div>
        ...
        {profile?.subscription_tier === "free" && (
          <div className="mt-10 bg-aubergine rounded-sm p-8 flex flex-col md:flex-row justify-between items-center gap-4">
            <div>
              ...
              <p className="text-offwhite/60 text-sm font-light">
                Unlimited AI summaries, offline reading, full past paper library. KSh 249/month.
              </p>
            </div>
